...
--2. 집합 연산자
--2-1. 합집합 연산자
--2000년 이후에 임용된 교수와 2000년 이후에 입사한 직원 목록 조회
SELECT PNO AS 직원번호
    ,  PNAME AS 직원이름
    ,  HIREDATE AS 입사일시
    FROM PROFESSOR
...
--2-2.차집합 연산자
-- 성이 제갈이면서 지원업무를 하지 않는 사원번호,사원이름,업무 조회
SELECT ENO
    ,  ENAME
    ,  JOB
    FROM EMP
...
-- 과목중에서 담당교수가 배정되지 않았거나, 담당교수의 정보가 NULL인 과목 번호,과목이름 조회
SELECT C.CNO
    ,  C.CNAME
    FROM COURSE C
MINUS
...
--2-3. 교집합 연산자
--물리, 화학과 학생 중 학점이 3.0 이상인 학생번호,학생이름,학과,평점 목록 조회
SELECT SNO
    ,  SNAME
    ,  MAJOR
    ,  AVR
