...
SELECT MAJOR, SNAME FROM STUDENT WHERE MAJOR = '유공' OR MAJOR = '생물' OR MAJOR = '식영';

--2) 화학과가 아닌 학생중에 1학년 학생을 검색하라
SELECT SNAME, MAJOR FROM STUDENT WHERE MAJOR != '화학';

--3) 물리학과 3학년 학생을 검색하라
SELECT SYEAR,SNAME,MAJOR FROM STUDENT WHERE SYEAR = 3 AND MAJOR = '물리'; 

--4) 평점이 2.0에서 3.0사이인 학생을 검색하라
SELECT AVR, SNAME FROM STUDENT WHERE AVR BETWEEN 2.0 AND 3.0;

--5) 교수가 지정되지 않은 과목중에 학점이 3학점인 과목을 검색하라
SELECT * FROM COURSE WHERE PNO IS NULL AND ST_NUM = 3;

--6) 화학과 관련된 과목중 평점이 2학점 이하인 과목을 검색하라
SELECT CNAME, ST_NUM FROM COURSE WHERE ST_NUM <= 2;
